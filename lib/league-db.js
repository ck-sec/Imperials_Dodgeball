...
const { validateBookingSchedule } = require('../js/league-schedule');
...
async function readWorld(sql) {
  ...
  const [row] = await sql`
    SELECT jsonb_build_object(
      ...
      'results', COALESCE((SELECT jsonb_agg(r ORDER BY r.event_id, r.player_id)
        FROM league_results r JOIN league_events e ON e.id = r.event_id
        WHERE e.cancelled_at IS NULL), '[]'::jsonb),
...
const ROSTER_SQL = `
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'rating', ROUND(COALESCE(p.initial_rating, CASE WHEN p.is_rookie THEN s.rookie_rating ELSE s.default_rating END)
      + COALESCE((SELECT SUM(r.rating_delta) FROM league_results r
        JOIN league_events e ON e.id = r.event_id
        WHERE r.player_id = p.id AND e.cancelled_at IS NULL), 0), 6)
  ) ORDER BY p.id), '[]'::jsonb)
...
function snapshotPlayerForEvent(world, event, player) {
  ...
  const before = candidate => {
    ...
    for (let index = 0; index < target.length; index++) {
      if (candidateKey[index] !== target[index]) return candidateKey[index] < target[index];
    }
    return false;
  };
  const priorEventIds = new Set(world.events.filter(candidate =>
    candidate.status === 'finalized' && before(candidate)).map(candidate => candidate.id));
  const delta = world.results.filter(result =>
    result.player_id === player.id && priorEventIds.has(result.event_id))
    .reduce((sum, result) => sum + Number(result.rating_delta), 0);
  return { ...player, rating: Math.round((Number(player.initial_rating) + delta) * 1e6) / 1e6 };
}
...
function bookingScheduleGuard(sql, session, schedule) {
  try {
    validateBookingSchedule(session, schedule);
  } catch (error) {
    throw new LeagueError(400, error.message);
  }
  // Check the current locked row, not the earlier world snapshot. Epoch arithmetic never wraps at midnight.
  return sql`SELECT league_assert(EXISTS(
    SELECT 1 FROM training_sessions
    WHERE id = ${session.id} AND start_time IS NOT NULL AND end_time IS NOT NULL
      AND start_time < end_time AND end_time < TIME '24:00:00'
      AND ${schedule.meetup_time}::time >= start_time
      AND EXTRACT(EPOCH FROM ${schedule.meetup_time}::time)
        + (${schedule.available_minutes}::numeric + ${schedule.finale_minutes}::numeric) * 60
        <= EXTRACT(EPOCH FROM end_time)
  ), 409, 'Training booking changed or the entire program, including the finale, no longer fits. Check start/end times in the Training tab, then reload and regenerate the schedule.')`;
}
...
async function applyAction(sql, input, world, actor = { is_admin: true }) {
  ...
  if (event.cancelled_at && input.action !== 'restore_matchday') {
    assert(false, 'Cancelled matchdays are read-only until restored', 409);
  }
  if (!event.cancelled_at && input.action === 'restore_matchday') {
    assert(false, 'This matchday is not cancelled', 409);
  }
  ...
  if (['save_teams', 'save_draft'].includes(input.action)) {
  ...
  } else if (['add_late_player', 'correct_lineup'].includes(input.action)) {
    assert(['published', 'finalized'].includes(event.status),
      input.action === 'add_late_player'
        ? 'Last-minute players can only be added after teams are published'
        : 'Live lineup corrections require published or finalized teams', 409);
    assert(world.sessions.some(session => session.id === event.session_id && !session.is_cancelled),
      'Cancelled training is read-only', 409);
    const currentIds = event.teams.flatMap(team => team.players.map(player => player.id));
    assert(currentIds.length <= 500 && new Set(currentIds).size === currentIds.length,
      'The saved roster is invalid', 409);
    const requested = input.action === 'add_late_player'
      ? event.teams.map(team => ({
        number: team.number,
        player_ids: [...team.players.map(player => player.id),
          ...(team.number === input.team_number ? [input.player_id] : [])],
      }))
      : input.teams;
    if (input.action === 'add_late_player') {
      assert(event.teams.some(team => team.number === input.team_number), 'Team not found', 404);
      assert(currentIds.length < 500, 'The saved roster is already at the 500-player limit', 409);
      assert(!currentIds.includes(input.player_id)
        && !world.results.some(result => result.event_id === event.id && result.player_id === input.player_id),
      'Player is already assigned to this training', 409);
    }
    const savedNumbers = event.teams.map(team => team.number).sort((a, b) => a - b);
    const requestedNumbers = requested.map(team => team.number).sort((a, b) => a - b);
    assert(JSON.stringify(requestedNumbers) === JSON.stringify(savedNumbers),
      'Live corrections must retain every existing team number', 409);
    const requestedIds = requested.flatMap(team => team.player_ids);
    assert(requestedIds.length <= 500 && new Set(requestedIds).size === requestedIds.length,
      'Each retained player must belong to exactly one team, with at most 500 players', 409);
    assert(requested.every(team => team.player_ids.length >= 1),
      'A live correction cannot leave a fixture team without players', 409);
    const eligible = playerView(world, { ...settings({}), ...event.settings });
    const currentSnapshots = new Map(event.teams.flatMap(team =>
      team.players.map(player => [player.id, player])));
    const additions = eligible.filter(player => requestedIds.includes(player.id) && !currentSnapshots.has(player.id))
      .sort((a, b) => a.id.localeCompare(b.id));
    assert(requestedIds.every(id => currentSnapshots.has(id) || additions.some(player => player.id === id)),
      'A selected player is merged, inactive, or unavailable', 404);
    const additionSnapshots = new Map(additions.map(player => {
      const { user_id, ...snapshot } = snapshotPlayerForEvent(world, event, player);
      return [player.id, snapshot];
    }));
    const requestedByNumber = new Map(requested.map(team => [team.number, team.player_ids]));
    const teams = event.teams.map(team => ({
      ...team,
      players: requestedByNumber.get(team.number).map(id => currentSnapshots.get(id) || additionSnapshots.get(id)),
    }));
    const rosterIds = [...requestedIds].sort();
    const awards = bonusAwards({ ...event, teams }, pruneBonusAwards(event, teams));
    let scored = null;
    if (event.status === 'finalized') {
      requireSeasonTwoScoringReady(world.seasons.find(season => season.id === event.season_id));
      assert(event.teams.every(team => Number.isInteger(Number(team.placement))
        && Number(team.placement) >= 1 && Number(team.placement) <= event.teams.length),
        'Finalized team placements are incomplete', 409);
      scored = scoreEvent({ ...event, teams, bonus_points: awards }, event.teams.map(team => ({
        team_number: team.number, placement: Number(team.placement),
      })));
    }
    queries.push(rosterLock(sql, event.session_id, rosterIds));
    queries.push(...eventGuard(sql, event.id, input.version, ['published', 'finalized']));
    queries.push(...sessionGuard(sql, event.session_id, event.season_id, event.session_date));
    queries.push(selectedPlayersGuard(sql, rosterIds));
    if (additions.length) {
      const frozenRosterSql = ROSTER_SQL.replaceAll('s.rookie_rating', '$4::numeric').replaceAll('s.default_rating', '$5::numeric');
      queries.push(sql.query(`SELECT league_assert((${frozenRosterSql}) = $3::jsonb, 409,
        '${input.action === 'add_late_player'
          ? 'The late player or their rating changed. Reload and try again.'
          : 'An added player or their rating changed. Reload and try again.'}')`,
      [additions.map(player => player.id), event.season_id, JSON.stringify(additions),
        Number(event.settings.rookie_rating), Number(event.settings.default_rating)]));
      queries.push(sql`UPDATE league_players p SET initial_rating = (snapshot->>'initial_rating')::numeric, updated_at = NOW()
        FROM jsonb_array_elements(${JSON.stringify(additions)}::jsonb) snapshot
        WHERE p.id = (snapshot->>'id')::uuid AND p.initial_rating IS NULL`);
    ...
    if (scored) {
      ...
      queries.push(sql`UPDATE league_events SET teams = ${JSON.stringify(scored.teams)}::jsonb,
        roster_ids = ${JSON.stringify(rosterIds)}::jsonb, bonus_points = ${JSON.stringify(awards)}::jsonb,
        roster_locked = true,
        version = version + 1, updated_at = NOW() WHERE id = ${event.id}`);
    } else {
  ...
  } else if (input.action === 'cancel_matchday') {
    assert(['published', 'finalized'].includes(event.status),
      'Only a published or finalized matchday can be cancelled', 409);
    queries.push(...eventGuard(sql, event.id, input.version, ['published', 'finalized']));
    queries.push(sql`SELECT league_assert(
      (SELECT cancelled_at IS NULL FROM league_events WHERE id = ${event.id}),
      409, 'This matchday is already cancelled')`);
    queries.push(sql`DELETE FROM league_match_timers WHERE event_id = ${event.id}`);
    queries.push(sql`UPDATE league_events SET cancelled_at = NOW(), cancelled_by = ${actor.user_id || null}::uuid,
      version = version + 1, updated_at = NOW() WHERE id = ${event.id}`);
  } else if (input.action === 'restore_matchday') {
    assert(['published', 'finalized'].includes(event.status),
      'Only a previously published matchday can be restored', 409);
    assert(world.sessions.some(session => session.id === event.session_id && !session.is_cancelled),
      'Restore the underlying training before restoring its matchday', 409);
    queries.push(...eventGuard(sql, event.id, input.version, ['published', 'finalized']));
    queries.push(...sessionGuard(sql, event.session_id, event.season_id, event.session_date));
    queries.push(sql`SELECT league_assert(
      (SELECT cancelled_at IS NOT NULL FROM league_events WHERE id = ${event.id}),
      409, 'This matchday is not cancelled')`);
    queries.push(sql`UPDATE league_events SET cancelled_at = NULL, cancelled_by = NULL,
      version = version + 1, updated_at = NOW() WHERE id = ${event.id}`);
    ...
    if (event.schedule) {
      queries.push(bookingScheduleGuard(sql, world.sessions.find(s => s.id === event.session_id), event.schedule));
    }
    ...
    queries.push(...eventGuard(sql, event.id, input.version, ['draft']));
    queries.push(...sessionGuard(sql, event.session_id, event.season_id, event.session_date));
    queries.push(bookingScheduleGuard(sql, world.sessions.find(s => s.id === event.session_id), schedule));
  ...
  await leagueTransaction(sql, queries);
  return { event_id: event.id, ...(input.action === 'add_late_player'
    ? { player_id: input.player_id, team_number: input.team_number } : {}) };
}
