...
async function applyAction(sql, input, world, actor = { is_admin: true }) {
  ...
  if (['save_teams', 'save_draft'].includes(input.action)) {
  ...
  } else if (input.action === 'add_late_player') {
    assert(['published', 'finalized'].includes(event.status),
      'Last-minute players can only be added after teams are published', 409);
    ...
    assert(currentIds.length < 500 && new Set(currentIds).size === currentIds.length,
      'The saved roster is invalid or already at the 500-player limit', 409);
    ...
    const selectedTeam = event.teams.find(team => team.number === input.team_number);
    assert(selectedTeam, 'Team not found', 404);
    const eligible = playerView(world, { ...settings({}), ...event.settings });
    const addition = eligible.find(player => player.id === input.player_id);
    assert(addition, 'Player is merged, inactive, or unavailable', 404);
    const { user_id, ...snapshot } = snapshotPlayerForEvent(world, event, addition);
    const teams = event.teams.map(team => team.number === input.team_number
      ? { ...team, players: [...team.players, snapshot] } : { ...team });
    const rosterIds = [...currentIds, input.player_id].sort();
    let scored = null;
    if (event.status === 'finalized') {
      ...
      scored = scoreEvent({ ...event, teams }, event.teams.map(team => ({
    ...
    [[input.player_id], event.season_id, JSON.stringify([addition]),
      Number(event.settings.rookie_rating), Number(event.settings.default_rating)]));
    queries.push(sql`UPDATE league_players SET initial_rating = ${addition.initial_rating}, updated_at = NOW()
      WHERE id = ${input.player_id} AND initial_rating IS NULL`);
    if (scored) {
      ...
      queries.push(sql`UPDATE league_events SET teams = ${JSON.stringify(scored.teams)}::jsonb,
        roster_ids = ${JSON.stringify(rosterIds)}::jsonb, roster_locked = true,
        version = version + 1, updated_at = NOW() WHERE id = ${event.id}`);
    } else {
      queries.push(sql`UPDATE league_events SET teams = ${JSON.stringify(teams)}::jsonb,
        roster_ids = ${JSON.stringify(rosterIds)}::jsonb,
        version = version + 1, updated_at = NOW() WHERE id = ${event.id}`);
    }
