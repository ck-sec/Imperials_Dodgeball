...
const EXTERNAL_REFEREE_POLICY = 'external_ref_v1';
...
function buildSchedule(teamCount, settings = {}) {
  ...
  const recommendedMatch = teamCount === 2 ? 60 : teamCount === 3 ? 30 : teamCount === 4 ? 15 : 17;
  const recommendedBreak = teamCount === 2 ? 0 : teamCount === 3 ? 7 : teamCount === 4 ? 3 : 5;
  ...
  const activeCourts = teamCount === 2 ? 1 : Math.min(courts, Math.floor((teamCount - 1) / 2));
  ...
  if (durationMinutes > availableMinutes) {
    throw new MatchError(`This match schedule needs ${durationMinutes} minutes from meetup, but games must finish by minute ${availableMinutes}. Shorten games or changeovers, or use the recommended team/court setup.`);
  }
  return {
    ...
    referee_policy: teamCount === 2 ? EXTERNAL_REFEREE_POLICY : REFEREE_POLICY, rounds
  };
}
...
function matchStandings(teamCount, schedule) {
  ...
  const externalReferee = schedule.referee_policy === EXTERNAL_REFEREE_POLICY;
  if (externalReferee && teamCount !== 2) {
    throw new MatchError('External-ref schedules are only valid for two teams.');
  }
  ...
  for (const round of schedule.rounds) {
    ...
    } else if (externalReferee) {
      if (round.referee_team !== null || !Array.isArray(round.bye_teams) || round.bye_teams.length
        || !Array.isArray(round.rest_teams) || round.rest_teams.length) {
        throw new MatchError('Two-team schedules must use an external referee without team byes.');
      }
    }
  }
