...
function timerView(world, eventId, requestedMatch, identity = {}, row, now = new Date(), dateNow = today()) {
  ...
  assert(event && !event.cancelled_at && ['published', 'finalized'].includes(event.status),
    'Published match not found', 404);
  ...
  return {
    ...
    match: {
      ...
      external_referee: visible.schedule.referee_policy === 'external_ref_v1',
    },
