...
function validateAction(body) {
  ...
  if (action === 'generate') {
    ...
    if (body.player_ids !== undefined) {
      assert(Array.isArray(body.player_ids) && body.player_ids.length >= 4 && body.player_ids.length <= 500,
        'Manual player_ids must contain 4–500 players');
...
function requirePublishableRoster(event) {
  ...
  assert(event.teams.length >= 2 && event.teams.length <= (event.max_teams || 5)
    && sizes.every(size => size >= event.team_size),
  'Every squad must meet the on-court team size before publishing', 409);
...
module.exports = {
  ...
  draftTeams, requirePublishableRoster, publicEvent,
};
