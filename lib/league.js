...
const { buildSchedule, matchStandings, resolvePlacements } = require('./league-matches');

const DEFAULTS = Object.freeze({
  placement_points: [3, 2.5, 2, 1, 0.5],
...
function settings(input) {
  ...
  assert(['relative', 'fixed'].includes(mode), 'scoring_mode must be relative or fixed');
  ...
  assert(points.every((p, i) => i === 0 || p <= points[i - 1]), 'Placement points must be non-increasing');
  assert(mode === 'fixed' || points.every(p => Math.abs(p / step - Math.round(p / step)) < 1e-8),
    'Relative placement points must all be multiples of points_step');
  return {
...
function scheduleSettings(input) {
  return {
    courts: number(input.courts === undefined ? 2 : input.courts, 'courts', 1, 2, true),
    match_minutes: number(input.match_minutes === undefined ? 20 : input.match_minutes, 'match_minutes', 1, 60, true),
    break_minutes: number(input.break_minutes === undefined ? 5 : input.break_minutes, 'break_minutes', 0, 30, true),
    available_minutes: number(input.available_minutes === undefined ? 120 : input.available_minutes, 'available_minutes', 1, 480, true),
  };
}
...
function validateAction(body) {
  ...
  if (action === 'generate') {
    ...
    assert(['auto', 4, 5, 6].includes(body.team_size), 'team_size must be auto, 4, 5 or 6');
    ...
    if (body.player_ids !== undefined) {
      assert(Array.isArray(body.player_ids) && body.player_ids.length >= 8 && body.player_ids.length <= 500,
        'Manual player_ids must contain 8–500 players');
  ...
  assert(['save_teams', 'publish', 'unpublish', 'results', 'generate_schedule', 'save_match', 'reopen_results'].includes(action), 'Invalid action');
  out.event_id = uuid(body.event_id, 'event_id');
  out.version = number(body.version, 'version', 1, 2147483646, true);
  if (action === 'generate_schedule') return { ...out, ...scheduleSettings(body) };
...
function chooseTeamSize(count, requested) {
  number(count, 'Attendee count', 8, 500, true);
  assert(['auto', 4, 5, 6].includes(requested), 'Invalid team size');
  const size = requested === 'auto'
    ? [6, 5, 4].find(n => count >= n * 2 && count % n === 0)
      || [6, 5, 4].find(n => count >= n * 2)
    : requested;
  assert(count >= size * 2, `At least ${size * 2} eligible players are needed for ${size}-a-side`);
  return size;
}

function balanceTeams(players, requested, maxTeams = 5) {
  const teamSize = chooseTeamSize(players.length, requested);
  ...
  const count = Math.min(maxTeams, Math.floor(players.length / teamSize));
...
function matchData(event) {
  ...
  const config = scheduleSettings(source);
  ...
  const rounds = source.rounds.map((round, i) => {
    const start = i * (config.match_minutes + config.break_minutes);
    ...
    return {
      number: round.number, start_minute: round.start_minute, end_minute: round.end_minute, bye_teams: byes,
  ...
  return { schedule: { ...config, duration_minutes: source.duration_minutes, rounds }, match_standings: table };
}
...
function editTeams(event, input) {
  ...
  return input.map(t => ({ number: t.number, name: t.name, placement: null,
    players: t.player_ids.map(id => ({ ...roster.get(id) })) })).sort((a, b) => a.number - b.number);
}
  ...
  teams.forEach((team, i) => {
    ...
    for (const player of team.players) {
      ledger.push({ player_id: player.id, display_name: player.display_name, team_number: team.number,
        placement: team.placement, points, rating_delta: delta });
    }
  });
  return { teams, ledger };
}
...
function publicEvent(event, session) {
  ...
  let endTime = session.end_time || null;
  if (!endTime && matches.schedule && /^([01]\d|2[0-3]):[0-5]\d(?::[0-5]\d)?$/.test(session.start_time || '')) {
    const [hours, minutes, seconds = 0] = session.start_time.split(':').map(Number);
    const end = (hours * 3600 + minutes * 60 + seconds + matches.schedule.duration_minutes * 60) % 86400;
    endTime = [Math.floor(end / 3600), Math.floor(end / 60) % 60, end % 60]
      .map(value => String(value).padStart(2, '0')).join(':');
  }
  return {
    id: event.id, season_id: event.season_id, session_id: event.session_id,
    title: session.title, session_date: session.session_date, start_time: session.start_time,
    end_time: endTime,
...
function publicView(world, requested, userId, dateNow = today()) {
  ...
  function accumulate(target, r) {
    const row = target.get(r.player_id) || { id: r.player_id, display_name: r.display_name, points: 0, played: 0, wins: 0 };
  ...
  function rankTotals(target) {
    for (const row of target.values()) row.points = Math.round(row.points * 1e6) / 1e6;
  ...
  const payload = {
    ...
    season: season ? { ...seasonSummary(season), placement_points: [...season.placement_points] } : null,
  ...
  if (userId) {
    ...
    payload.stats = { ...(own ? { rank: own.rank, points: own.points, played: own.played, wins: own.wins }
      : { rank: null, points: 0, played: 0, wins: 0 }), ...movement(own) };
...
function adminView(world) {
  ...
  return {
    ...
    sessions: world.sessions.map(s => ({ id: s.id, title: s.title, session_date: s.session_date,
      start_time: s.start_time, location: s.location, is_cancelled: s.is_cancelled,
    ...
    events: world.events.map(e => {
      ...
      return { id: e.id, season_id: e.season_id, session_id: e.session_id, title: session.title,
        session_date: session.session_date, start_time: session.start_time, location: session.location,
...
module.exports = {
  ...
  requireEditableRoster, eventPlacements, buildSchedule,
};
