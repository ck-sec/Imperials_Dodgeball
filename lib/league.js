...
const { REFEREE_POLICY, buildSchedule, matchStandings, resolvePlacements } = require('./league-matches');
...
function scheduleSettings(input, refereeSafe = true, preserveRecommendedTiming = false) {
  const meetupTime = input.meetup_time === undefined ? (refereeSafe ? '18:00' : null) : input.meetup_time;
  assert(meetupTime === null || (typeof meetupTime === 'string' && /^([01]\d|2[0-3]):[0-5]\d$/.test(meetupTime)),
    'meetup_time must use HH:MM');
  const warmupMinutes = number(input.warmup_minutes === undefined ? (refereeSafe ? 15 : 0) : input.warmup_minutes,
  ...
  return {
    ...
    match_minutes: input.match_minutes === undefined && preserveRecommendedTiming ? undefined
      : number(input.match_minutes === undefined ? (refereeSafe ? 17 : 20) : input.match_minutes, 'match_minutes', 1, 60, true),
    ...
    finale_minutes: number(input.finale_minutes === undefined ? (refereeSafe ? 10 : 0) : input.finale_minutes,
      'finale_minutes', refereeSafe ? 1 : 0, 10, true),
  };
}
...
function validateAction(body) {
  ...
  if (action === 'generate') {
    ...
    out.max_teams = number(body.max_teams === undefined ? 5 : body.max_teams, 'max_teams', 3, 5, true);
    if (body.player_ids !== undefined) {
      assert(Array.isArray(body.player_ids) && body.player_ids.length >= 6 && body.player_ids.length <= 500,
        'Manual player_ids must contain 6–500 players');
  ...
  assert(['save_teams', 'save_draft', 'save_bonus_points', 'set_bonus', 'publish', 'unpublish', 'results', 'generate_schedule', 'save_match', 'reopen_results'].includes(action), 'Invalid action');
...
function balanceTeams(players, requested, maxTeams = 5, refereeSafe = false) {
  number(maxTeams, 'max_teams', refereeSafe ? 3 : 2, 5, true);
  const teamSize = refereeSafe ? refereeTeamSize(players.length, requested, maxTeams) : chooseTeamSize(players.length, requested);
  ...
  const count = refereeSafe ? (feasible >= 5 ? 5 : 3) : feasible;
...
function matchData(event) {
  ...
  const config = scheduleSettings(source, refereeSafe);
  ...
  const activeCourts = Math.min(config.courts, Math.floor((event.teams.length - 1) / 2));
  if (refereeSafe) {
  ...
  return {
    schedule: {
      ...config,
      active_courts: refereeSafe ? activeCourts : config.courts,
      duration_minutes: source.duration_minutes,
      finale_start_minute: refereeSafe ? config.available_minutes : null,
      total_duration_minutes: refereeSafe ? config.available_minutes + config.finale_minutes : source.duration_minutes,
      referee_policy: refereeSafe ? REFEREE_POLICY : null,
      rounds,
    },
...
function requirePublishableRoster(event) {
  ...
  if (event.schedule) {
    assert(event.schedule.referee_policy === REFEREE_POLICY,
      'Regenerate the schedule before publishing so every slot has a referee team', 409);
    matchData(event);
  }
...
function requireFinaleAwards(event) {
  if (event.schedule?.referee_policy === REFEREE_POLICY) finaleAwardCategories(event, true);
}

function finaleResults(event) {
  if (event.status !== 'finalized' || event.schedule?.referee_policy !== REFEREE_POLICY) return null;
