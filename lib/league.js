...
function validateAction(body) {
  ...
  assert(['save_teams', 'save_draft', 'save_bonus_points', 'set_bonus', 'publish', 'unpublish',
    'results', 'generate_schedule', 'delete_schedule', 'save_match', 'reopen_results',
    'add_late_player', 'correct_lineup', 'cancel_matchday', 'restore_matchday'].includes(action), 'Invalid action');
  out.event_id = uuid(body.event_id, 'event_id');
  out.version = number(body.version, 'version', 1, 2147483646, true);
  if (action === 'add_late_player') {
    return { ...out, player_id: uuid(body.player_id, 'player_id'),
      team_number: number(body.team_number, 'team_number', 1, 125, true) };
  }
  if (action === 'correct_lineup') {
    ...
    out.teams = body.teams.map(team => {
      ...
      assert(Array.isArray(team.player_ids) && team.player_ids.length >= 1 && team.player_ids.length <= 500,
        'Every team must retain 1–500 players');
      return {
        number: number(team.number, 'Team number', 1, 125, true),
        player_ids: team.player_ids.map(id => uuid(id, 'Player id')),
      };
    });
    const ids = out.teams.flatMap(team => team.player_ids);
    assert(ids.length <= 500, 'The corrected roster may contain at most 500 players');
    assert(new Set(out.teams.map(team => team.number)).size === out.teams.length,
      'Corrected teams contain duplicate team numbers');
    assert(new Set(ids).size === ids.length, 'Each player must belong to exactly one team');
    return out;
  }
...
function teammateSummaries(results, eventIds, eventOrder) {
  const teams = new Map();
  const byPlayer = new Map();
  for (const result of results) {
    if (!eventIds.has(result.event_id)) continue;
    const key = `${result.event_id}:${result.team_number}`;
    if (!teams.has(key)) teams.set(key, []);
    teams.get(key).push(result);
    if (!byPlayer.has(result.player_id)) byPlayer.set(result.player_id, new Map());
  }
  for (const team of teams.values()) {
    const unique = [...new Map(team.map(result => [result.player_id, result])).values()];
    for (const player of unique) {
      const teammates = byPlayer.get(player.player_id);
      for (const other of unique) {
        if (other.player_id === player.player_id) continue;
        const row = teammates.get(other.player_id) || {
          id: other.player_id, display_name: other.display_name,
          played_together: 0, wins_together: 0, name_order: -1,
        };
        const order = eventOrder.get(player.event_id) ?? -1;
        if (order >= row.name_order) {
          row.display_name = other.display_name;
          row.name_order = order;
        }
        row.played_together++;
        row.wins_together += player.placement === 1 ? 1 : 0;
        teammates.set(other.player_id, row);
      }
    }
  }
  return new Map([...byPlayer].map(([playerId, teammates]) => [playerId,
    [...teammates.values()].map(({ name_order, ...row }) => row).sort((a, b) =>
      b.played_together - a.played_together || b.wins_together - a.wins_together
...
function publicView(world, requested, userId, dateNow = today()) {
  ...
  const released = world.events.filter(e => !e.cancelled_at && ['published', 'finalized'].includes(e.status));
  ...
  if (userId) {
    ...
    payload.player_linked = Boolean(ownPlayer);
    const ownResults = world.results.filter(r => ownPlayer && r.player_id === ownPlayer.id && eventMap.has(r.event_id));
    payload.teammates = ownPlayer
      ? (teammateSummaries(world.results, new Set(eventMap.keys()), eventOrder).get(ownPlayer.id) || [])
        .map(({ id, ...row }) => row)
      : [];
    ...
    for (const result of ownResults) {
      ...
      placementTotal += result.placement;
    }
    ...
    payload.performance = ownResults.length ? {
      average_placement: Math.round(placementTotal / ownResults.length * 100) / 100,
      best_placement: Math.min(...ownResults.map(result => result.placement)),
      podiums: ownResults.filter(result => result.placement <= 3).length,
      placement_counts: placementCounts,
    } : { average_placement: null, best_placement: null, podiums: 0, placement_counts: {} };
    payload.history = ownResults.map(r => {
...
function adminStatisticsView(world, requested, dateNow = today()) {
  const season = selectSeason(world.seasons, requested, dateNow);
  const seasons = world.seasons.map(publicSeasonSummary);
  if (!season) {
    return { seasons, season: null, standings: [], players: [], comparison_event: null, events: [] };
  }

  const hasReleasedEvent = world.events.some(event =>
    event.season_id === season.id && !event.cancelled_at && ['published', 'finalized'].includes(event.status));
  const payload = hasReleasedEvent ? publicView(world, season.id, undefined, dateNow) : {
    season: publicSeasonSummary(season),
    guide_season: publicSeasonSummary(season),
    standings: [],
    comparison_event: null,
    events: [],
  };
  const sessions = new Map(world.sessions.map(session => [session.id, session]));
  const finalized = world.events.filter(event =>
    event.season_id === season.id && !event.cancelled_at && event.status === 'finalized')
    .sort((a, b) => {
      const first = sessions.get(a.session_id), second = sessions.get(b.session_id);
      return first.session_date.localeCompare(second.session_date)
        || (first.start_time || '').localeCompare(second.start_time || '') || a.id.localeCompare(b.id);
    });
  const eventOrder = new Map(finalized.map((event, index) => [event.id, index]));
  const finalizedIds = new Set(finalized.map(event => event.id));
  const teammates = teammateSummaries(world.results, finalizedIds, eventOrder);
  const summaries = new Map();
  for (const result of world.results) {
    if (!finalizedIds.has(result.event_id)) continue;
    const row = summaries.get(result.player_id) || {
      id: result.player_id, display_name: result.display_name, points: 0, bonus_points: 0,
      played: 0, wins: 0, podiums: 0, placement_total: 0, placement_counts: {}, name_order: -1,
    };
    const order = eventOrder.get(result.event_id);
    if (order >= row.name_order) {
      row.display_name = result.display_name;
      row.name_order = order;
    }
    row.points += Number(result.points);
    row.bonus_points += Number(result.bonus_points || 0);
    row.played++;
    row.wins += result.placement === 1 ? 1 : 0;
    row.podiums += result.placement <= 3 ? 1 : 0;
    row.placement_total += result.placement;
    row.placement_counts[result.placement] = (row.placement_counts[result.placement] || 0) + 1;
    summaries.set(result.player_id, row);
  }
  const players = [...summaries.values()].sort((a, b) => b.points - a.points
    || a.display_name.localeCompare(b.display_name) || a.id.localeCompare(b.id));
  let rank = 0;
  players.forEach((row, index) => {
    ...
    row.bonus_points = Math.round(row.bonus_points * 1e6) / 1e6;
    row.base_points = Math.round((row.points - row.bonus_points) * 1e6) / 1e6;
    row.average_placement = Math.round(row.placement_total / row.played * 100) / 100;
    row.best_placement = Math.min(...Object.keys(row.placement_counts).map(Number));
    row.win_rate = Math.round(row.wins / row.played * 1000) / 10;
    row.teammates = teammates.get(row.id) || [];
    if (index === 0 || row.points !== players[index - 1].points) rank = index + 1;
    row.rank = rank;
    delete row.name_order;
    delete row.placement_total;
  });
  return { ...payload, seasons, season: publicSeasonSummary(season), players, finalized_event_count: finalized.length };
}

function playerView(world, season = selectSeason(world.seasons)) {
  const activeEventIds = new Set(world.events.filter(event => !event.cancelled_at).map(event => event.id));
  return world.profiles.filter(p => !p.merged_into && (p.user_id === null
    ...
    const delta = world.results.filter(r => r.player_id === profile.id && activeEventIds.has(r.event_id))
      .reduce((sum, r) => sum + Number(r.rating_delta), 0);
  ...
  return {
    ...
    events: world.events.map(e => {
      ...
      return { id: e.id, season_id: e.season_id, session_id: e.session_id, title: session.title,
        ...
        is_cancelled: Boolean(e.cancelled_at), cancelled_at: e.cancelled_at || null,
...
module.exports = {
  ...
  chooseTeamSize, balanceTeams, editTeams, scoreEvent, selectSeason, publicView, adminStatisticsView, playerView, adminView,
